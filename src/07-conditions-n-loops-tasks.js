...
function isBracketsBalanced(str) {
  ...
  const res = str.split('').reduce((prev, cur) => {
    if (bracketPairs[cur] && prev[prev.length - 1] === bracketPairs[cur]) {
      prev.pop();
    } else {
      prev.push(cur);
    }
...
function getCommonDirectoryPath(pathes) {
  const resArr = [];
  if (pathes.every((i) => i[0] === '/')) {
    const arr = pathes.sort((a, b) => a.length - b.length).map((i) => i.split('/'));
    arr[0].forEach((i, idx) => {
      const item = i;
      if (arr.every((p) => p[idx] === item)) {
        resArr.push(item);
      }
    });
    return resArr.join('/').replace(/$/, '/');
  }
  return '';
}
